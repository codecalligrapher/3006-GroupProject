...
//    }
//    LED_TEST_TWO    =   1;
    //eeprom_write(0, EE_LOC_INTERVAL);
#endif    
#if NDEBUG
    LED_TEST_ONE    =   1;
#endif
    
    if(eeprom_read(EE_LOC_INTERVAL) == 0)
    {
    ...
    if(eeprom_read(EE_LOC_STATUS) == 1)
    {
#if NDEBUG
        LED_TEST_ONE    =   1;
#endif
        sprintf(LCD_rate, "BPM: %u", eeprom_read(EE_LOC_TEMP_RATE));
        sprintf(LCD_var, "pNN50: %i%%" ,eeprom_read(EE_LOC_TEMP_VAR));
        sprintf(LCD_t, "TEMP: %i.%i", eeprom_read(EE_LOC_TEMP_TEMP_INT), eeprom_read(EE_LOC_TEMP_TEMP_FRAC));
        sprintf(LCD_gluc, "GLUCOSE: %i", eeprom_read(EE_LOC_TEMP_GLUC));
        eeprom_write(0,EE_LOC_STATUS);
        display_stats();
        dly_8s();
#if NDEBUG
        LED_TEST_ONE    =   0;
        ...
        while(!get_action_valid())
        {
          ...
          LED_TEST_TWO    =   0;
           ...
           LED_TEST_ONE    =   1;
                    ...
                    sprintf(LCD_t, "TEMP: %i.%i", temp_get_int(),temp_get_fraction());
                    sprintf(LCD_gluc, "GLUCOSE:%img/dl", adc_glucose_get());
                    ...
                    LED_TEST_TWO    =   1;
#endif
                    eeprom_write(1, EE_LOC_STATUS);
                    eeprom_write(countv_get_rate(),EE_LOC_TEMP_RATE);
                    eeprom_write(pnn50_get(),EE_LOC_TEMP_VAR);
                    eeprom_write(temp_get_int(),EE_LOC_TEMP_TEMP_INT);
                    eeprom_write(temp_get_fraction(),EE_LOC_TEMP_TEMP_FRAC);
                    eeprom_write(adc_glucose_get(),EE_LOC_TEMP_GLUC);  
#if NDEBUG
                    LED_TEST_TWO    =   0;
#endif
                    speaker_alarm();
    ...
    LED_TEST_ONE  =  0;
                ...
                case 'B':
                    ...
                    {                        
                        ...
                        eeprom_write(1, EE_LOC_STATUS);
                        eeprom_write(eeprom_read(addr_scroll),EE_LOC_TEMP_RATE);
                        eeprom_write(eeprom_read(addr_scroll + 0x01),EE_LOC_TEMP_VAR);
                        eeprom_write(eeprom_read(addr_scroll + 0x02),EE_LOC_TEMP_TEMP_INT);
                        eeprom_write(eeprom_read(addr_scroll + 0x03),EE_LOC_TEMP_TEMP_FRAC);
                        eeprom_write(eeprom_read(addr_scroll + 0x04),EE_LOC_TEMP_GLUC);   
                        kpad_config();
                        Delay1KTCYx(256);
                        Delay1KTCYx(256);
                        while(!PORTBbits.INT1);
